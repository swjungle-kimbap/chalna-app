...
import { LoginResponse, SignUpResponse } from '../../interfaces';
...
export const SignUpByWithKakao = async (deviceId:string, fcmToken:string) :Promise<LoginResponse | null>=> {
  try {
    ...
    const singUpResponse = await axiosPost<SignUpResponse>(
      urls.SIGNUP_URL, "회원 가입 요청", signUpRequestBody);
    await setKeychain('loginToken', singUpResponse.data.loginToken);
    await KakaoLogin.logout();
    return logIn(singUpResponse.data.loginToken, deviceId, fcmToken);
    } catch (error:any) {
